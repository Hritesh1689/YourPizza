...
class MainViewModel() : ViewModel() {
    ...
    private val totalPizzaCountInCart: MutableLiveData<String> =
        MutableLiveData<String>().apply { value = "0" }
    ...
    fun setTotalPizzaCounts() {
        ...
        if (pizzHashMap.containsKey(key.toString())) {
            ...
            if (openSizeSelector.value != null)
                newAddedPizza = PizzaInCart(
                    openSizeSelector.value?.id!!,
                    openSizeSelector.value?.name!!,
                    currentPrice.value?.name!!,
